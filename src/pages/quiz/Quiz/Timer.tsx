...
export function Timer({endTime, ...props}: Props): React.ReactElement {
	if (endTime === null)
...
function Inner({startTime, endTime, onExpired}: InnerProps): React.ReactElement {
	...
	React.useEffect(() => {
		timerRef.current = window.setInterval(() => {
			...
			// 2) Due to #1, submitting when equal to zero, as opposed to afterward, means that an automatically
